#include "SanUSB48X.h" //Firmware para controle RGB via bluetooth - Recomendável um capacitor de 10uF ou 100uF entre Vcc(5V) e Gnd.
...
if (serial_interrompeu)  { INTCONbits.TMR0IE = 0;//Desabilita interupção timer 0
     ...
     if(comando[n]==10 && (comando[1]==',' || comando[2]==',' || comando[3]==',')){ // Garantiu o fim da string
	unsigned char r=0,g=0,b=0; //zerar as variáveis globais do RGB
	int i=0,cont=0;
        inverte_saida(pin_b7);
	for(i=0;i<12;i++){
        	if (comando[i] == ','){++cont;++i;}

		if (cont == 0){comand[1]=r*10+comando[i]-0x30;}
		if (cont == 1){comand[2]=g*10+comando[i]-0x30;}
		if (cont == 2){
			       if (comando[i] == 10){break;}  //10=?/n? ->delimitador
                               comand[3]=b*10+comando[i]-0x30;
                      	      }
                         }
        comando[1] = 32;comando[2] = 32;comando[3] = 32;
	//printf(" r = %i \n",r);//para debuggar
	//printf(" g = %i \n",g);
	//printf(" b = %i \n",b);
	n=0;
